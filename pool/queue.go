...
func (wq *loopQueue) offer(worker *goWorker) error {
	if wq.size == 0 {
...
func (wq *loopQueue) pop() *goWorker {
	if wq.isEmpty() {
		return nil
	}
...
func (wq *loopQueue) getExpires(expireTime int64) []*goWorker {
	index := wq.binarySearch(expireTime)
	if index == -1 {
		return nil
	}
...
func (wq *loopQueue) binarySearch(expireTime int64) int {
	var mid, nlen, basel, tmid int
	nlen = len(wq.items)

	// if no need to remove work, return -1
	if wq.isEmpty() || wq.items[wq.head].recycleTime > expireTime {
		return -1
	}
	...
	for l <= r {
		...
		if expireTime < wq.items[tmid].recycleTime {
			r = mid - 1
		} else {
			l = mid + 1
		}
